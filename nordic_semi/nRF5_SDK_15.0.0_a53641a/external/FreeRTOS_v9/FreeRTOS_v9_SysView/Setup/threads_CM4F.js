...
{  
  ...
  var lr = TargetInterface.peekWord(sp);
  WScript.Echo("lr "+lr.toString(16)+"\n");
  sp+=4;
  if ((lr & 0x10)!=0x10)  // FP has been saved    
    sp+=4*16
  for (i=0;i<4;i++)
    {
      a[i] = TargetInterface.peekWord(sp);  
...
function update() 
{
  ...
  if (list)
  {
    list = Debug.evaluate("*(List_t *)" + list);
  ...
  {
    list = Debug.evaluate("*(List_t *)" + list);
    add_list(list, "blocked, overflowed");
  }
